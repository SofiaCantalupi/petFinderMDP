...
public class JwtService {
    ...
    // Esta es una KEY secreta que tenemos que guardar. Con esta key se generan
    // todos los Tokens y al tenerla secreta nos aseguramos
    // que los tokens no puedan ser trucados. Solo con esta key se pueden generar y
    // validar tokens LEGITIMOS.
    // Por seguridad se guarda en una variable de entorno para no exponerla.
    private final String SECRET_KEY;

    // Lee desde el properties el valor de la key, que esta vinculada a una variable
    // de entorno
    // definida en el IDE de cada uno (JWT_SECRET)
    // El metodo de abajo hace que se guarde en el string secretKey la variable de
    // entorno del IDE y se guarde en SECRET_KEY
    public JwtService(@Value("${jwt.secret}") String secretKey) {
        this.SECRET_KEY = secretKey;
    }

    // Este metodo convierte la constante SECRET_KEY en un objeto de clase Key que
    // se usa para "crear y validar" (firmar y verificar) los tokens
    private Key getSignInKey() {
        return Keys.hmacShaKeyFor(SECRET_KEY.getBytes());
    }

    // Generación del token sin especificaciones.
    // Este token va a ser el que use el usuario para navegar por la aplicación web
    // una vez logeado.
    // Este token nomás contiene el mail del usuario.
    public String generateToken(UserDetails userDetails) {
        return Jwts.builder() // builder inicializa la construccion del token
                .setSubject(userDetails.getUsername()) // Llama al metodo getUsername del userDetails, que en realidad
                                                       // obtiene el mail del miembro
    ...
    // Metodo para validar que el usuario logeado coincida con el del token, que no
    // este expirado y que la cuenta siga activa
    public Boolean validarToken(String token, UserDetails userDetails) {
        final String email = extraerEmail(token);
        return (email.equals(userDetails.getUsername()) && !esTokenExpirado(token) && userDetails.isEnabled());
    }
